...
static uint32_t ALIGN_ALPH_SIZE(updateMtx[ALPH_SIZE][ALPH_SIZE]);
...
static size_t encChunk(uint32_t[CDF_SIZE], unsigned char *, size_t, const unsigned char *, size_t);
...
static void modUpdate(uint32_t[CDF_SIZE], unsigned char);
...
//internal functions
static size_t
encChunk(uint32_t cdf[CDF_SIZE], unsigned char *out, size_t out_size, const unsigned char *in, size_t in_size) {
    ...
    for (size_t i = in_size; i > 0; --i)
        if (encPut(&cod, &ptr, rng_buff[i - 1], out))
            return 0;
...
static int encPut(uint32_t *c, unsigned char **pptr, struct range rng, const unsigned char *lim) {
    uint32_t x = *c;
...
static int decPut(uint32_t *, unsigned char **, struct range, const unsigned char *);

static uint32_t decGet(const uint32_t *);

static unsigned char modSymb(const uint32_t[CDF_SIZE], uint32_t);
...
decChunk(uint32_t cdf[CDF_SIZE], unsigned char *out, const size_t out_size, const unsigned char *in,
    ...
    for (size_t i = 0; i < out_size; ++i) {
        ...
        if (decPut(&cod, &ptr, rng, &in[in_size]))
            return 0;
...
static int decPut(uint32_t *c, unsigned char **pptr, const struct range rng, const unsigned char *lim) {
    uint32_t x = *c;
...
static unsigned char modSymb(const uint32_t cdf[CDF_SIZE], const uint32_t prb) {
